...
:copyright: (c) 2015 by gdhe55555@gmail.com.
:license: BSD.
"""
...
def index():
    ...
    cur = g.db.execute('select username, message, time from message_board order by time desc')
    posts = [dict(username=row[0], message=row[1], time=row[2]) for row in cur.fetchall()]
